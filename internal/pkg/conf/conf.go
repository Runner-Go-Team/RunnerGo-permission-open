...
type Config struct {
	...
	CompanyOperationLogTime     int               `yaml:"companyOperationLogTime"`
}
...
func initCompanyOperationLogTime() {
	companyOperationLogTime, err := strconv.ParseInt(os.Getenv("RG_COMPANY_OPERATION_LOG_TIME"), 10, 64)
	if err != nil {
		Conf.CompanyOperationLogTime = 1
	} else {
		Conf.CompanyOperationLogTime = int(companyOperationLogTime)
	}
}
